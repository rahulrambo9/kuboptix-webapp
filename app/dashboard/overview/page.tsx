  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 border border-slate-800 bg-[#0a0f1c]/50 rounded text-center">
        <HealthMetric label="CLUSTER STATUS" value="HEALTHY" color="text-[#10b981]" />
        <HealthMetric label="HEALTHY NODES" value="3 / 3" color="text-white" />
  ...
  return (
    <div className="p-5 rounded border border-slate-800 bg-[#0a0f1c]/80 relative overflow-hidden group hover:border-opacity-50 transition-all" style={{ borderColor: `${color}30` }}>
      ...
      <div className="flex justify-between items-start mb-2">
         <h3 className="text-xs font-bold text-slate-400 tracking-widest">{title}</h3>
  ...
  return (
    <div className="p-6 rounded border border-slate-800 bg-[#0a0f1c]/50 h-80 flex flex-col relative">
       ...
       <div className="flex items-center gap-2 mb-1 text-white">
         <div style={{ color: color }}>{icon}</div>
         <h3 className="font-orbitron tracking-wider text-sm">{title}</h3>
       </div>
       <p className="text-xs text-slate-500 font-mono mb-6 ml-6">{subtitle}</p>
...
function HealthMetric({ label, value, color }: any) {
  return (
    <div className="flex flex-col items-center justify-center space-y-1">
      ...
      <span className={`font-orbitron font-bold text-lg ${color}`}>{value}</span>
    </div>
  );
}
